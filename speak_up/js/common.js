	...
	win[namespace] = {
		...
		askQuestion: function(script, question){
			...
			if (question === undefined){
				...
				win[namespace].checkAnswer(script, question, fnEndBack);
				// fnEndBack();
			}
		},
		/**
		...
		checkAnswer: function(script, question, endBack){
			...
			if (question.type === 'word') {
				...
				$btnVoide.addEventListener('click', function(){
					...
					if (voiceText.reduceText.slice(0, reduceAnswerText.length) !== reduceAnswerText) {
						...
						// 1트에 실패일 시, 초성 나옴
						if (tryNum === 1){
							win[namespace].setText(win[namespace].wrongScript[0].text);
							win[namespace].wrongAnswer(
								win[namespace].wrongScript[0], 
								function(){
							...
							win[namespace].setText(win[namespace].wrongScript[1].text);
							win[namespace].wrongAnswer(
								win[namespace].wrongScript[1], 
								function(){
							...
							win[namespace].setText(win[namespace].wrongScript[2].text);
							win[namespace].wrongAnswer(// [To-be] wrongAnswer()가 아니라, 으음~아쉬워 알려줄게! 랜덤으로
								win[namespace].wrongScript[2], 
								function(){ 
								}
							);
